...
def build_ctx(data: dict, m: dict) -> dict:
    ...
    return {
        ...
        "q1_skill_count":    len(m["q1_skills"]) if m.get("q1_skills") is not None and not (hasattr(m["q1_skills"], "empty") and m["q1_skills"].empty) else 0,
        "q2_skill_count":    len(m["q2_skills"]) if m.get("q2_skills") is not None and not (hasattr(m["q2_skills"], "empty") and m["q2_skills"].empty) else 0,
